...
export default function OnboardingPage() {
  ...
  // useWallets from @privy-io/react-auth/solana returns only Solana wallets
  ...
  return (
    <div className="min-h-screen flex flex-col p-6" data-screen-label="Onboarding">
      <Link href="/" className="flex items-center gap-3 mb-8 self-start">
        <BrandMark size={32} />
        <span className="font-brand font-medium text-base tracking-[0.04em]">VoiceFi</span>
      </Link>
      ...
      {step === 1 && (
        <div className="max-w-[480px] w-full mx-auto bg-[var(--card)] border border-[var(--border)] rounded-[28px] p-9 px-9 animate-fade-in-up">
          ...
          <p className="text-[var(--muted-foreground)] mb-7 text-base">
            Sign in with your email or Google. We&apos;ll create a secure account for you in the background &mdash; no
            passwords or seed phrases to remember.
          </p>

          <Button variant="outline" className="w-full" onClick={handleLogin} disabled={loading || !ready}>
          ...
          <Button
            ...
            onClick={handleLogin}
            disabled={loading || !ready || (email !== "" && !email.includes("@"))}
          >
            {loading ? "Opening…" : "Continue"} <ArrowRight size={18} />
          </Button>
      ...
      {step === 2 && (
        <div className="max-w-[480px] w-full mx-auto bg-[var(--card)] border border-[var(--border)] rounded-[28px] p-9 animate-fade-in-up">
          ...
          <p className="text-[var(--muted-foreground)] mb-7 text-base text-center">
            Your secure account is ready. Tap the microphone on the next screen and say something like &ldquo;check my
            balance.&rdquo;
          </p>
          ...
          <div className="bg-[var(--input)] rounded-2xl py-3.5 px-4.5 px-[18px] flex items-center gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="mono text-sm font-medium overflow-hidden whitespace-nowrap text-ellipsis">
                {walletAddress ? truncateAddress(walletAddress) : "Creating account…"}
              </div>
            </div>
            <Button
              variant="ghost"
